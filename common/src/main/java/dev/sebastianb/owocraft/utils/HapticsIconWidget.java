...
public class HapticsIconWidget extends AbstractWidget {
    ...
    protected void renderWidget(GuiGraphics guiGraphics, int mouseX, int mouseY, float f) {
        ...
        switch (OwocraftClient.getConnectionStateManager().getState()) {
            case CONNECTING -> location = CommonOwocraft.id("textures/gui/connection/connecting.png");
            case CONNECTED -> location = CommonOwocraft.id("textures/gui/connection/connected.png");
            case DISCONNECTED -> location = CommonOwocraft.id("textures/gui/connection/disconnected.png");
        }
